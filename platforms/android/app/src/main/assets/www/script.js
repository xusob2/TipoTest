...
const adminDeleteListSection = document.getElementById;
const confirmationModal = document.getElementById('confirmation-modal');
const modalConfirmBtn = document.getElementById('modal-confirm-btn');
const modalCancelBtn = document.getElementById('modal-cancel-btn');('admin-delete-list-section');
...
async function startQuiz(moduleName, questionsToUse = null, isReview = false) {
    ...
    if (questionsToUse) {
        ...
        try {
            ...
            console.log(`[API] Preguntas cargadas exitosamente para ${moduleName}: ${currentQuizQuestions.length} preguntas.`);
        } catch (error) {
...
async function retryFailedQuestions(failedIndices) {
    ...
    questionsToRetry.sort(() => Math.random() - 0.5);
